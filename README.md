...
* **Cơ chế chống Spam/Nhiễu (Debounce & Cooldown):**
    * **Debounce:** Yêu cầu thiết bị nằm trong vùng kích hoạt ít nhất X giây để lọc nhiễu sóng GPS.
    * **Cooldown lock:** POI sau khi phát xong sẽ bị khóa (Lock) trong vòng Y phút để không lặp lại tiếng.

### 3.3. Thuyết minh tự động (Narration)
* Tích hợp công nghệ **Text-to-Speech (TTS)** bằng `Microsoft.Maui.Media.TextToSpeech`.
* Linh hoạt, nhỏ nhẹ, hỗ trợ đọc các file văn bản thành ngôn ngữ tự nhiên.

### 3.4. Quản lý dữ liệu & Hàng chờ
* **Priority Queue:** Xử lý hàng chờ nếu lỡ đi ngang 2 điểm POI gần nhau cùng lúc -> điểm nào Priority cao hơn sẽ được phát trước.
* **Duck Volume:** Áp dụng luồng tiến trình tự động dừng/nhường khi có cuộc gọi nội bộ (OS-level interruptions).

### 3.5. Bản đồ (Map View UI)
* Tích hợp bản đồ mã nguồn mở **OpenStreetMap** (thông qua Mapsui.Maui).
* Tracking vị trí thực tế của người dùng thời gian thực (Blue dot marker).
* Đổ toàn bộ danh sách POI từ SQLite thành Red Marker (Ghim đỏ).
...
* Giao diện Admin quản trị ứng dụng mô hình MVC, kết nối với Database thông qua **Entity Framework Core**.
* **Hệ thống CRUD:** Thêm, sửa, xóa, quản trị danh sách Điểm tham quan (POI), quản lý thư viện Audio và nội dung.
...
* **Hệ thống API Đồng bộ:**
    ...
    * `POST /api/analytics/history`: Mobile tự động báo cáo ẩn danh dữ liệu người dùng.
* **Thống kê Dashboard (Web):** Tính năng Heat Map đánh giá mức độ hấp dẫn của các POI.

### 4.3. Kích hoạt nội dung bổ sung (QR Code)
* Cho phép người dùng trực tiếp mở Camera App tích hợp để quyét mã QR dán tại trạm -> Bỏ qua Geofence -> Đẩy thẳng vào hàng đợi TTS Audio.
...
* **Hiệu năng:** Tốc độ phản hồi từ khi chạm vùng Radius Geofence đến lệnh Fire Audio phải <= 2 giây.
* **Design Pattern:** Code áp dụng **Dependency Injection (DI)**, **Singleton Service** (cho Audio/GPS) và kiến trúc **MVVM**.
