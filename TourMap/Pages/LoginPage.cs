...
public class LoginPage : ContentPage
{
        ...
        var registerLink = new Label
        {
            FormattedText = new FormattedString
            {
                Spans =
                {
                    ...
                    new Span { Text = "Đăng ký", TextColor = Color.FromArgb("#1565C0"), FontSize = 14, FontFamily = "InterSemiBold", TextDecorations = TextDecorations.Underline }
                }
            },
            HorizontalOptions = LayoutOptions.Center,
    ...
    private async Task ForgotPasswordAsync()
    {
        string? email = await DisplayPromptAsync(
            ...
            -1,
            Keyboard.Email,
            _emailEntry.Text?.Trim() ?? string.Empty);
