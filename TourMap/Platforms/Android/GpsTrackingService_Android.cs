...
public class GpsTrackingService_Android : IGpsTrackingService, IDisposable
{
    ...
    private const double MaxAccuracyMeters = 80.0; // Lọc GPS noise
    private const double FirstFixMaxAccuracyMeters = 250.0; // Allow a coarse first dot so user sees location quickly
    ...
    public async Task StartTrackingAsync()
    {
        ...
        try
        {
            ...
            if (status != PermissionStatus.Granted)
            {
                ...
                if (status != PermissionStatus.Granted)
                {
                    Console.WriteLine("[GpsTracking] Foreground location permission denied.");
            ...
            // (Không bắt buộc để chạy foreground; nếu từ chối vẫn chạy tiếp.)
            // ═══════════════════════════════════════════════════════════
            ...
            // Chạy vòng lặp tracking trên background thread (non-blocking)
            _trackingTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        Console.WriteLine($"[GpsTracking] Lỗi khởi tạo GPS lần đầu: {ex.Message}");
                    }
                ...
                {
                    Console.WriteLine($"[GpsTracking] Background loop crashed: {ex.Message}");
                }
            }, token);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[GpsTracking] Error starting tracking: {ex.Message}");
            Interlocked.Exchange(ref _isTrackingFlag, 0);
        }
    }
    ...
    private async Task PollLocationAsync()
    {
        try
        {
            ...
            // Lọc nhiễu: vẫn cho phép first-fix có sai số cao hơn để chấm xanh xuất hiện sớm.
            if (location.Accuracy.HasValue && location.Accuracy.Value > MaxAccuracyMeters)
            {
                var canUseAsFirstFix = LastKnownLocation == null && location.Accuracy.Value <= FirstFixMaxAccuracyMeters;
                if (!canUseAsFirstFix)
                    return;
            }
