...
public class GeofenceEngine
{
    ...
    public void OnLocationChanged(Location userLocation)
    {
        ...
        lock (_lock)
        {
            // Dọn dẹp từ khóa cache đã hết hạn mỗi 15 phút để chống Memory Leak
            if ((now - _lastCleanupTime).TotalMinutes >= 15)
            {
                var expiredKeys = _cooldowns
                    .Where(kvp => (now - kvp.Value).TotalMinutes >= CooldownDurationMinutes)
                    .Select(kvp => kvp.Key)
                    .ToList();
                ...
                Console.WriteLine($"[Geofence] 🧹 Cleaned up {expiredKeys.Count} expired cooldowns.");
            }
