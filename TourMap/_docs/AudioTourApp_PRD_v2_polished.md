...
4. [Hệ thống App Người dùng (Ứng dụng di động)](#4-hệ-thống-app-người-dùng-mobile-app)
5. [Chiến lược Đồng bộ & Offline First](#5-chiến-lược-đồng-bộ--offline first) *(CẬP NHẬT)*
6. [Hệ thống Admin — CMS Web](#6-hệ-thống-admin--cms-web)
7. [Thiết kế Cơ sở Dữ liệu](#7-thiết-kế-cơ-sở-dữ-liệu)
8. [Thiết kế RESTful API](#8-thiết-kế-rest-api)
9. [Yêu cầu Phi chức năng](#9-yêu-cầu-phi-chức-năng)
10. [Kiến trúc Triển khai & Môi trường](#10-kiến-trúc-triển-khai--môi-trường) *(CẬP NHẬT)*
...
Hệ thống gồm 3 thành phần chính: **Ứng dụng di động** (người dùng cuối), **Backend API**, và **Admin Web CMS**.
...
graph TB
 ...
 direction TB
 API["RESTful API<br/>SyncController | AuthController"]
...
|---|---|---|
| Ứng dụng di động | .NET MAUI 8.0 | Ứng dụng Android/iOS — GPS, Geofence, TTS, Map, QR |
...
| Backend API | ASP.NET Core 8 | RESTful API — CRUD POI, Analytics, Auth, Sync |
...
Geofence Engine là thành phần lõi của ứng dụng — liên tục so sánh vị trí người dùng với danh sách POI để quyết định khi nào cần phát nội dung.
...
flowchart TD
 ...
 F -->|Không| G["Bỏ qua — tránh kích hoạt lặp"]
...
sequenceDiagram
 ...
 Nar->>Nar: Bỏ qua (tránh kích hoạt lặp)
 end
```
...
stateDiagram-v2
 ...
 PAUSED --> PLAYING : Hết cuộc gọi / Chọn tiếp tục
...
### 4.1 Sơ đồ chi tiết Ứng dụng di động
...
graph TB
 ...
 SETTINGS[" Cài đặt<br/>- Chọn ngôn ngữ<br/>- Độ nhạy GPS / Bán kính<br/>- Chọn giọng TTS<br/>- Tải gói offline"]
 end
...
| **Settings** *(CẬP NHẬT)* | **Chọn ngôn ngữ TTS, tốc độ nói, bán kính, gói offline** | **Preferences API + local storage** |
| **Offline Pack** *(CẬP NHẬT)* | **Tải gói audio + POI để dùng offline** | **Background download + SQLite sync** |
...
### 4.6 Màn hình Cài đặt & Tùy chỉnh *(CẬP NHẬT)*
...
## 5. CHIẾN LƯỢC ĐỒNG BỘ & OFFLINE-FIRST *(CẬP NHẬT)*
...
sequenceDiagram
 ...
 Note over App,CDN: === Lần đầu mở ứng dụng (Full Sync) ===
...
graph TB
 ...
 subgraph TOUR_MGMT["️ Quản lý Tour (CẬP NHẬT)"]
...
- File audio: upload mp3/m4a, xem trước trực tiếp
...
stateDiagram-v2
 ...
 [*] --> ClickAddPOI : Chọn "Thêm mới Địa điểm"
 ...
 SaveToDatabase --> TriggerMobileUpdate : Gửi cờ thay đổi xuống Ứng dụng di động
 TriggerMobileUpdate --> [*]
```
...
- Tạo TTS xem trước ngay trên web (Azure/Google Cognitive API)
...
sequenceDiagram
 ...
 Admin->>CMS: Chọn [Tạo âm thanh AI] (Chọn giọng Nữ, Tiếng Việt)
...
### 6.6 Module Quản lý Tour *(CẬP NHẬT)*
...
| Tiêu chí | Yêu cầu |
|---|---|
| Thời gian khởi động ứng dụng | ≤ 3 giây (cold start) |
...
### 9.4 Khả dụng & Trải nghiệm (UX/Accessibility) *(CẬP NHẬT)*
...
## 10. KIẾN TRÚC TRIỂN KHAI & MÔI TRƯỜNG *(CẬP NHẬT)*
...
graph TB
 ...
 API_APP["RESTful API<br/>Port 5001"]
 end
...
|---|---|---|---|
| **Development** | Dev cá nhân, debug | SQLite local | hỗ trợ gỡ lỗi nhanh, debug mode |
...
| Phase 3 — Backend | Tuần 5–6 | RESTful API, Database, File Storage, JWT Auth, Sync API | Backend deployed |
...
| Phase 6 — Hoàn thiện | Tuần 11–12 | UI/UX polish, **Settings UI**, performance tuning, testing, docs | Final demo |
